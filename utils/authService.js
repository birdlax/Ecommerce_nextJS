...
export async function fetchApi(endpoint, options = {}) {
  ...
  if (response.status === 204) {
    ...
    if (!response.ok && response.status !== 204) {
      // response.ok จะ false สำหรับ 204 ในบาง client
      console.error(
        `[fetchApi] API request for ${url} returned ${response.status} ${response.statusText} (expected 204)`
      );
      throw new Error(
        `API request ${response.statusText} with status ${response.status}`
      );
    }
  ...
  if (response.headers.get("content-length") === "0" && !response.ok) {
    console.error(
      `[fetchApi] API request for ${url} returned ${response.status} ${response.statusText} with no content`
    );
    throw new Error(
      `API request ${response.statusText} with status ${response.status} and no content`
    );
  }
  ...
  try {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if (!response.ok) {
        console.error(
          `[fetchApi] API Error (Non-JSON Response) for ${url}: Status ${
            response.status
          }, Body: ${textData.substring(0, 100)}...`
        );
        throw new Error(
          textData || `API request failed with status ${response.status}`
        );
      }
    ...
    if (!response.ok) {
      console.error(
        `[fetchApi] API response was not OK and failed to parse for ${url}:`,
        response.status,
        response.statusText,
        parseError
      );
      try {
        ...
        throw new Error(
          `API request failed with status ${response.status}: ${response.statusText} (and response body could not be read)`
        );
      }
    }

    console.error(
      `[fetchApi] API response was OK but not valid JSON for ${url}:`,
      parseError
    );
...
export const loginUser = async (credentials) => {
  ...
  return fetchApi("/login", {
    // สมมติ endpoint คือ /login ตามที่คุณให้มา
    method: "POST",
...
export const getCurrentUser = async () => {
  try {
    ...
    // ใช้ fetchApi กลาง ซึ่งมันจะจัดการเรื่อง Token และ Header ให้เอง
    const data = await fetchApi("/profile/me");
    return data;
  } catch (error) {
    console.error("[authService] getCurrentUser failed:", error.message);
    // ไม่ต้อง throw error ก็ได้ถ้าการที่ user ไม่ได้ login เป็นเรื่องปกติ
    return null;
  }
};
