...
export const adminGetAllUsers = async (page = 1, limit = 10) => {
  ...
  console.log(
    `[adminService] Admin: Getting all users. Page: ${page}, Limit: ${limit}`
  );
...
export const adminGetUserById = async (userId) => {
  ...
  console.log(
    `[adminService] Admin: Getting user by ID: ${userId} from /admin/user/${userId}`
  );
...
export const adminUpdateUserProfileById = async (userId, userData) => {
  if (!userId)
    throw new Error("User ID is required for adminUpdateUserProfileById");

  console.log(
    `[adminService] Admin: Updating user ID: ${userId} with data:`,
    userData
  );
...
export const adminGetAllOrders = async (
  ...
  console.log(
    `[adminService] Admin: Getting all orders. Page: ${page}, Limit: ${limit}, Sort: ${sortField} ${sortOrder}`
  );

  return fetchApi(
    `/admin/orders?page=${page}&limit=${limit}&sort=${sortField}&order=${sortOrder}`
  );
};
...
export const adminUpdateOrder = async (orderId, updateData) => {
  ...
  console.log(
    `[adminService] Admin: Updating order ID: ${orderId} with data:`,
    updateData
  );
...
export const adminGetRevenueReport = async (
  year,
  month,
  compareYear,
  compareMonth
...
export const adminGetSalesTrend = async (period = "daily", range = 30) => {
  console.log(
    `[adminService] Admin: Getting sales trend, period: ${period}, range: ${range}`
  );
  ...
  let queryParam =
    period === "daily" ? "days" : period === "weekly" ? "weeks" : "months";

  return fetchApi(
    `/admin/reports/sales-trend?period=${period}&${queryParam}=${range}`
  );
};
...
export const adminGetRecentProducts = async (limit = 5) => {
  ...
  return fetchApi(
    `/admin/products?sort=CreatedAt&order=desc&limit=${limit}&page=1`
  );
};
